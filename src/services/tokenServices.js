...
export const sendToken = (req, res, user, statusCode, message, deletionToken = null) => {
    try {
        ...
        const cookieOptions = {
            ...
            expires: new Date(Date.now() + parseInt(config.JWT_COOKIE_EXPIRE) * 24 * 60 * 60 * 1000)
        }
        ...
        logger.info(responseMessage.TOKEN_SUCCESS(userData.name))
    } catch (error) {
        ...
        httpError(handleError, error, req, res, 500)
    }
}
